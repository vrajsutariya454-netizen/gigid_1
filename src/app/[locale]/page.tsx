  ...
  return (
    <header className="mx-auto w-full max-w-7xl px-6 py-8 flex items-center justify-between">
      <div className="flex items-center gap-3 group cursor-pointer">
        ...
        <span className="font-sans font-bold text-xl tracking-tight text-foreground/90">{appT('name')}</span>
      </div>

      <nav className="hidden md:flex items-center gap-8 text-sm font-medium text-muted-foreground">
        <a href="#" className="hover:text-foreground transition-colors">{t('features') || 'Features'}</a>
        <a href="#" className="hover:text-foreground transition-colors">{t('security') || 'Security'}</a>
        <a href="#" className="hover:text-foreground transition-colors">{t('enterprise') || 'Enterprise'}</a>
        <a href="#" className="hover:text-foreground transition-colors">{t('pricing') || 'Pricing'}</a>
      </nav>
      ...
      <div className="grid lg:grid-cols-[1.15fr_0.85fr] gap-16 lg:gap-24 items-center">
        ...
        >
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-muted border border-border backdrop-blur-md">
            ...
            <span className="text-[11px] font-bold uppercase tracking-[0.15em] text-muted-foreground">{appT('beta_notice') || 'Now in private beta'}</span>
          </div>

          <h1 className="font-display text-[clamp(4.5rem,8vw,6.5rem)] text-gradient tracking-tight leading-[0.9] mt-2">
            {appT('hero_title_1') || 'Your work,'} <br />
            <span className="italic font-normal">{appT('hero_title_2') || 'your identity.'}</span>
          </h1>
          ...
          <div className="flex flex-wrap items-center gap-4 pt-4">
            <button className="group relative px-8 py-4 rounded-xl bg-primary text-white font-bold tracking-widest text-sm transition-all hover:scale-105 active:scale-95 shadow-lg shadow-primary/25">
              <span className="flex items-center gap-2 uppercase">
                {appT('claim_hub') || 'Claim your hub'} <ArrowUpRight className="w-5 h-5 transition-transform group-hover:translate-x-1 group-hover:-translate-y-1" />
              </span>
            </button>
            <button className="px-8 py-4 rounded-xl glass border-border text-foreground/70 font-bold tracking-widest text-sm transition-all hover:bg-muted hover:text-foreground uppercase">
              {appT('watch_demo') || 'Watch the demo'}
            </button>
          </div>
...
function StatsStrip() {
  ...
  const stats = [
    { value: "$2.4B", labelKey: "earnings_tracked" },
    { value: "184k", labelKey: "verified_workers" },
    ...
    <div className="grid grid-cols-3 gap-8 pt-12 border-t border-border w-full mt-8">
      {stats.map((stat, i) => (
        <div key={i} className="flex flex-col gap-1">
          ...
          <span className="text-[10px] uppercase tracking-[0.2em] text-muted-foreground font-bold">{t(stat.labelKey)}</span>
        </div>
      ))}
    </div>
  );
  ...
  return (
    <div className="noise glass-card rounded-[2rem] p-10 lg:p-12 shadow-2xl relative overflow-hidden">
      <div className="flex justify-between items-start mb-10">
        <div className="flex flex-col gap-2">
          <h2 className="font-display text-4xl text-foreground tracking-tight">
            {isSignUp ? t('create_profile') : t('welcome_back')}
          </h2>
          <p className="text-sm text-muted-foreground font-medium">
            {isSignUp ? t('start_journey') : t('sign_in_desc')}
          </p>
        </div>
        {!isSignUp && (
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-6">
        ...
        {isSignUp && (
          <div className="grid gap-6">
            <div className="flex flex-col gap-2.5">
              <label className="text-[10px] font-bold uppercase tracking-[0.2em] text-muted-foreground/70 ml-1">{t('full_name')}</label>
              <input
            ...
            <div className="flex flex-col gap-2.5">
              <label className="text-[10px] font-bold uppercase tracking-[0.2em] text-muted-foreground/70 ml-1">{t('username')}</label>
              <input
        ...
        <div className="flex flex-col gap-2.5">
          <label className="text-[10px] font-bold uppercase tracking-[0.2em] text-muted-foreground/70 ml-1">{t('email')}</label>
          <input
        ...
        <div className="flex flex-col gap-2.5">
          <div className="flex justify-between items-center ml-1">
            <label className="text-[10px] font-bold uppercase tracking-[0.2em] text-muted-foreground/70">{t('password')}</label>
            <button type="button" className="text-[10px] font-bold uppercase tracking-[0.1em] text-primary hover:text-accent transition-colors">{t('forgot_password')}</button>
          </div>
          <input
        ...
        >
          {isLoading ? t('processing') : isSignUp ? t('create_account') : t('access_hub')}
      ...
      <div className="relative my-10">
        ...
        <div className="relative flex justify-center text-xs uppercase tracking-widest"><span className="bg-transparent px-4 text-muted-foreground/50 font-bold">{t('or')}</span></div>
      </div>
  ...
  return (
    <footer className="mx-auto w-full max-w-7xl px-6 py-10 flex flex-col md:flex-row items-center justify-between gap-6 border-t border-border mt-auto">
      ...
      <div className="flex items-center gap-8 text-[10px] font-bold uppercase tracking-[0.2em] text-muted-foreground/50">
        <a href="#" className="hover:text-foreground transition-colors">{t('security')}</a>
